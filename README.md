<h1 align="center">Hi 👋, I'm Abdullah Jahangir</h1>
<h3 align="center">Aspiring AI & Automation Engineer | Python | FastAPI | n8n | Agentic AI</h3>

---

### 📞 Contact Information
**Abdullah Jahangir**  
📍 Karachi, Pakistan  
📱 +92 317 0288875  
✉️ abdullah.vertex@gmail.com  

[![Portfolio](https://img.shields.io/badge/Visit%20Portfolio-00C896?style=for-the-badge&logo=vercel&logoColor=white)](http://abdullah-ai.lovable.app/)  
[![LinkedIn](https://img.shields.io/badge/LinkedIn-0077B5?style=for-the-badge&logo=linkedin&logoColor=white)](https://www.linkedin.com/in/abdullah-jahangir-vertex/)

---

### 📝 Summary
Dedicated AI & Data Science student with hands-on experience in Python programming, machine learning, real-world AI solutions, and workflow automation using **n8n**. Currently specializing in **Agentic AI**.  

Strong problem-solver, quick learner, and highly motivated to deliver efficient, high-quality technical solutions as an **AI & Automation Engineer**.

---

### 🛠️ Tech Skills & AI Stack

**Programming & Development**  
<img src="https://skillicons.dev/icons?i=python,fastapi,django,flask,postgres,mongodb" /><br/>

**AI & Machine Learning**  
<img src="https://skillicons.dev/icons?i=langchain,huggingface,ollama" />  
**Gemini • OpenAI • FAISS • YOLOv8**

**Automation & Tools**  
<img src="https://skillicons.dev/icons?i=n8n,docker,git,linux,vscode" /><br/>

**Web & Frontend**  
<img src="https://skillicons.dev/icons?i=html,css,js,react,tailwind" /><br/>

**Productivity**  
MS Office (Word, Excel, PowerPoint)

---

### 💼 Experience

**Co-Founder**  
**Evengers Tech Solution** — Present  
Karachi, Pakistan (Remote)  
- Building and delivering AI, automation, and web application solutions for clients  
- Developing backend APIs using **Python & FastAPI**  
- Creating workflow automations with **n8n**  
- Managing a 10-member team across software, AI, and web projects  

**AI & Automation Engineer**  
**Morphe Labs** — Present (Remote)  
- Designing and implementing **n8n-based** automation workflows  
- Integrating AI agents and LLMs into automation pipelines  
- Building end-to-end workflows using n8n, APIs, and Python scripts  
- Optimizing automations for speed, reliability, and scalability  

**AI & Data Science Intern**  
**Saylani Tech** — Oct 2025 — Jan 2026  
Karachi, Pakistan  
- Trained, tested, and optimized multiple ML & AI models  
- Developed backend microservices using **FastAPI**  
- Built AI-powered chatbot with **LLMs + RAG**  
- Integrated **Google Gemini API** and experimented with **Ollama** (local LLMs)  
- Assisted students in debugging Python, ML, and automation code  

---

### 🎓 Education
- **DAE in Computer Information Technology** — Pioneer Technical College  
- **Secondary School Certificate (SSC) in Science/Biology** — Ibrahim Ali Bhai Secondary School  

---

### 🏆 Certifications
- Agntic AI — Saylani Mass IT Training (SMIT)  
- AI and Database Science — Saylani Mass IT Training (SMIT)  
- MS Office — Alimagir Institute of Technology  

---

### 📌 Featured Projects
- **Shopify AI Customer Support Chatbot** — RAG + FAISS + text/voice/image support (Python + FastAPI)  
- **Smart People Counter** — Real-time system using **YOLOv8 + IoU Tracking**  
- **Crypto Assistant AI Agent** — Price tracking, news summarization & insights  
- **RAG Based AI System (n8n)** — Document-driven responses using RAG + n8n  
- **Adaptive Web Driven AI Chatbot** — Context-aware chatbot  
- **Web Typing Speed Application** — Real-time feedback web app  

*(Full project links available in my GitHub repositories)*

---

### 🤝 Connect with Me
<p align="left">
  <a href="mailto:abdullah.vertex@gmail.com"><img src="https://img.shields.io/badge/Gmail-D14836?style=for-the-badge&logo=gmail&logoColor=white"/></a>
  <a href="https://www.linkedin.com/in/abdullah-jahangir-vertex/" target="_blank"><img src="https://img.shields.io/badge/LinkedIn-0077B5?style=for-the-badge&logo=linkedin&logoColor=white"/></a>
  <a href="https://www.facebook.com/abdullah.jahangirai/" target="_blank"><img src="https://img.shields.io/badge/Facebook-1877F2?style=for-the-badge&logo=facebook&logoColor=white"/></a>
  <a href="https://www.instagram.com/abdullahjahangir.ai/" target="_blank"><img src="https://img.shields.io/badge/Instagram-E4405F?style=for-the-badge&logo=instagram&logoColor=white"/></a>
  <a href="https://github.com/abdullahjahangir123" target="_blank"><img src="https://img.shields.io/badge/GitHub-100000?style=for-the-badge&logo=github&logoColor=white"/></a>
</p>

---

<p align="center">
  <img src="https://komarev.com/ghpvc/?username=abdullahjahangir123&label=Profile+Views&color=00C896&style=flat" />
</p>
<p align="center">⭐ If you like my work,
